...
{
    register int i, j, k, msec;
    float cc;
    clock_t start, temps;
    float *MA, *MB, *MC;

    // TM saisie comme argument du main, sinon 1024 par defaut
    register const TM = argc > 1 ? atoi(argv[1]) : 1024;
    ...
    for (i = 0; i < TM; ++ i) {
        for (j = 0; j < TM; ++ j) {
            ...
            // M[i][j] = M[j][i]
            // M[j][i] = tmp
            const indice1 = i * TM + j;
            const indice2 = j * TM + i;
            const tmp = MB[indice1];
            MB[indice1] = MB[indice2];
            MB[indice2] = tmp;
        }
    }
